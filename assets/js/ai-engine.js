...
(function (global) {
  ...
  function buildBm25Index(chunks) {
    ...
    for (var i = 0; i < docs.length; i++) {
      var tf = docs[i].tf;
      for (var term in tf) {
        if (Object.prototype.hasOwnProperty.call(tf, term)) {
          df[term] = (df[term] || 0) + 1;
        }
      }
    }
  ...
  async function callPollinations(messages, temperature) {
    ...
    if (!res.ok) {
      ...
      if (res.status === 429) {
        throw new Error(
          "Pollinations HTTP 429: Rate limit exceeded or queue full. Consider configuring a custom AI Worker or WebLLM in _config.yml.",
        );
      }
      throw new Error(
        "Pollinations HTTP " +
          ...
          (errBody
            ? ": " + errBody
            : " (empty body; check network / ad blockers)"),
      );
    }
  ...
  async function callWebLlm(modelId, messages, onProgress, temperature) {
    ...
    try {
      mod =
        await import("https://cdn.jsdelivr.net/npm/@mlc-ai/web-llm@0.2.73/+esm");
    } catch (e1) {
    ...
    if (typeof CreateMLCEngine !== "function")
      throw new Error(
        "WebLLM CreateMLCEngine not available in this package build.",
      );
  ...
  async function callProvider(opts) {
    ...
    if (provider === "worker") {
      if (!cfg.workerUrl)
        throw new Error(
          "Worker URL not configured (site.ai_worker_url in _config.yml).",
        );
      return callWorker(
        cfg.workerUrl,
        cfg.workerModel || "gpt-4o-mini",
        messages,
        temperature,
      );
    }
