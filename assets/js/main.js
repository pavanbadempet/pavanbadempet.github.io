...
(function ($) {
	...
	const swup = new Swup({
		...
		plugins: [],
		cache: true,
		animationSelector: false // Disable waiting for animations since we removed the class
	});

	/*
		Aggressive Prefetching
	*/
	$('body').on('mouseenter', 'a[href^="/"], a[href^="' + window.location.origin + '"]', function () {
		var url = $(this).attr('href');
		if (url.indexOf('#') !== -1 || url.indexOf('mailto:') !== -1 || url.indexOf('tel:') !== -1) return;

		// Use fetch to warm up the browser cache
	...
	function init() {
		...
		*/
		...
		$('.logged-in .section.started').not('.section-title').css({ 'height': height - 32 });
		if (width < 783) {
			$('.section.started').not('.section-title').css({ 'height': height });
			$('.logged-in .section.started').not('.section-title').css({ 'height': height - 46 });
		}
		...
		*/
		if (width > 1199) {
			...
			$('a, .btn-group').off('mouseenter.cursor mouseleave.cursor');
			...
			$('a, .btn-group').on({
				'mouseenter.cursor': function (e) {
					cursor_over();
				},
				'mouseleave.cursor': function (e) {
					cursor_out();
				}
			});
		}
		...
		*/
		...
		/*
			Hover Button Effect
		*/
		$('.hover-animated .circle').off('mouseenter mouseleave'); // Unbind first
		...
		*/
		$('.filters').off('click'); // Unbind first
		...
		*/
		...
		/*
			Gallery popup
		*/
		$('.has-popup-gallery').off('click'); // Unbind first
		...
		*/
		$('.widget-title').each(function () {
			if ($(this).find('.widget-title-span').length) {
				return;
			}
